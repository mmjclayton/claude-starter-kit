...
You want to get involved more in "AI" and start levelling up your skills. You have a Windows PC, you're paying for Claude (or about to), and you want to get the most out of it. You're not a developer and you've never written code, or not much of it at least. You want to see what all the fuss is about.

With this starter guide, you'll set up two tools, in this order:

1. **Claude Cowork first** - a mode inside the Claude desktop app where custom skills live. This is the fastest way to get real productivity wins. Installing a skill and building your own takes about 30 minutes, no command line required.
2. **Claude Code next** - a command line tool for doing more powerful work with Claude directly from your PC's command line. Takes about an hour to set up cleanly from scratch, and the guide walks you through it with every step explained in plain English.
...
- Claude Code running in PowerShell (so you can start building things with Claude Code)
...
If you already have Claude installed on your Windows PC you can skip to Step 2.
...
3. Install the **Claude desktop app** from https://claude.ai/download. Run the installer, follow the prompts, launch it, and sign in with the account you just created.
...
**How to switch models in Claude Code:** type `/model` in PowerShell and pick from the list. Same idea - switch any time.

Default to Sonnet. Reach for Opus when you need more horsepower.

### A note on permissions

Cowork will ask your permission every time it wants to do something on your PC for the first time - access a folder, download a file, run a command. You'll see Allow/Deny prompts throughout your chats. This is normal and intentional. The walkthrough flags when to expect them. General rule: if the ask matches what you just requested, allow it.
